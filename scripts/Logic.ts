...
class Exchange{
    ...
    registerUser(username: string, role: "seller" | "buyer"): void {
    ...
    loginUser(username: string, role: "seller" | "buyer"): boolean {
    ...
    async setETHBalance(user: User, amount: number): Promise<boolean>{
        try{
    ...
    async getETHBalance(user: User): Promise<bigint> {
        let balance;
        try{
    ...
    async getAxelTokenBalance(user: User): Promise<bigint> {
    ...
    async mintAxelToken(user: User, amount: Number): Promise<boolean>{
    ...
    async startAuction(user: User, startingPrice: Number, reservePrice: Number, duration: Number): Promise<boolean>{
        if (user && user.role === "seller") {
            ...
            const token = user.sellerData.token;
            const balance = await this.getAxelTokenBalance(user);
    ...
    async transferToken(user: User, auction: DutchAuction): Promise<boolean>{
    ...
    async transferAllTokens(user: User): Promise<boolean>{
        if (user && user.role === "seller") {
    ...
    getAllAuctions(): DutchAuction[]{
        const auctions = this.users.flatMap(user => user.sellerData?.auction || []);
        return auctions;
    }

    // Function to join auction
    joinAuction(user: User, auction: DutchAuction): boolean{
    ...
    async placeBid(user: User, amount: number): Promise<boolean>{
        if (user && user.role == "buyer"){
            if (user.buyerData.auction == null) throw new Error("You have not joined an auction.");
            const auction = user.buyerData.auction;
            try{
